...
function describeRalplanBashBlock(cwd: string, command: string): string {
  ...
  if (blockedTarget && isUnresolvedVariableTarget(blockedTarget)) {
    return formatPlanningWriteBlockDetail("unresolved Bash write target", blockedTarget, RALPLAN_ALLOWED_WRITE_PREFIXES);
  }
  if (blockedTarget) {
    const operationClass = /\btee\s+(?:-a\s+)?/.test(command) ? "Bash tee write" : "Bash redirect write";
    return formatPlanningWriteBlockDetail(operationClass, blockedTarget, RALPLAN_ALLOWED_WRITE_PREFIXES);
  }
  return "Bash write intent did not identify an allowed planning artifact path; operation class Bash write; target <unresolved>";
}
...
async function buildRalplanPreToolUseBoundaryOutput(
  ...
  if (toolName === "Bash") {
    ...
    if (blocked) {
      blockedDetail = describeRalplanBashBlock(cwd, command);
    }
